...
show_board_art() {
  ...
  case "$PI_BOARD" in
   ...
   │  1› o o o ${HL}#${RST} ${HL}*${RST} o o o o o o o o o o o o o o ‹39          │
   │           │ │                                            │
   │           ├── ${HL}PIN 7 (GPIO4): antenna wire here${RST}           │
   │           │ └── ${HL}PIN 9 (GND): ground plane wire here${RST}      │
   │                                                    ┌─────┤
   │ ‹SD card                                           │ USB │
   │  (underside,                                       │ USB │
   │   this end)                  ┌──────┐              ├─────┤
...
EOF
   ...
   ┌──────────────────────────────────────────────────────────┐
   │ 39› o o o o o o o o o o o o o o o ${HL}*${RST} ${HL}#${RST} o o o ‹1           │
   │ 40› o o o o o o o o o o o o o o o o o o o o ‹2           │
   │                                   │ │                    │
   │     ${HL}PIN 9 (GND): ground plane  ${RST} ───┘ │                    │
...
EOF
   ...
   Generic 40-pin header (pin 1 is nearest the SD-card end):
    ...
    GPIO4 ${HL}›(7)‹${RST} (8)  GPIO14   ◀── ${HL}PIN 7 (GPIO4): antenna wire here${RST}
      GND ${HL}›(9)‹${RST} (10)  GPIO15  ◀── ${HL}PIN 9 (GND): ground plane wire here${RST}
           ... continues to (40)
EOF
      ;;
  esac
  cat <<EOF
   ...
   pin 9 (GND) -- directly below GPIO4 in the same column. Run it
   horizontally away from the board, roughly the same length as
   the antenna.
...
import dbus, gi, logging, os, re, time
...
def write_rds(ps: str = None, rt: str = None) -> None:
  ...
  pi_fm_rds (the FIFO reader) only runs while audio is flowing. A plain
  open() blocks until a reader appears, wedging this daemon; instead retry
  a non-blocking open briefly, then drop the update if no transmitter is
  listening (it re-announces on the next track change anyway).
  """
  data = ""
  if ps is not None: data += f"PS {ps}\n"
  if rt is not None: data += f"RT {rt}\n"
  if not data: return
  fd = None
  for _ in range(5):
    try:
