...
| `airplay-rds.service` | shairport-sync metadata → RDS FIFO; HTTP tuner UI on port 8750; optional volume-key frequency control |
...
**FIFO drain during frequency changes:** `airplay_announce.sh` must keep `/run/airplay_audio` drained while `airplay2fm.service` is stopped. If no process holds the read end open, shairport-sync gets SIGPIPE on its next write, drops the AirPlay session, and iOS takes ~60 s to reconnect (heard as silence). The fix: `cat "$AUDIO_PIPE" >/dev/null &` before stopping the service, then start the service, sleep 0.3 s for its `cat` to open the FIFO, then kill the drainer. The brief overlap (both `cat`s reading) is harmless — don't remove it.
