...
- `airplay-rds.py` also implements volume-key frequency control from the same pipe: `ssnc`/`pvol` events carry `"airplay_volume,volume,lowest,highest"` (first field −144 = mute, else −30..0 dB). Tuning works **only while paused** (a deliberate product decision — during playback the rocker is ordinary volume control and pvol changes just update the baseline). Clicks are **batched**: a worker thread applies the net change (clicks × `STEP`, clamped to `[FMIN, FMAX]`) after 3 s of quiet, then runs `airplay_announce.sh` once (LED flash + TTS on the old frequency, confirmation on the new, stop/start of `airplay2fm.service`). `bt-volume-freqd.sh` mirrors the same batching with a `read -t` poll loop. Hardware-learned constraints: iOS routes the rocker to AirPlay only while playing plus ~2–3 s after pausing, so users must click promptly (keep the pvol baseline persistent across pause/resume — resetting it eats the first click); `pi_fm_rds` never exits when its WAV ends, so announce transmissions must be wrapped in `timeout`; the sender emits no volume event when a press can't change the volume (already at max/mute). The metadata pipe must have exactly one reader — never split `pvol` handling into a second process reading the same FIFO.
- After tuning, `airplay-rds.py` undoes the volume movement via the DACP back-channel: it captures `ssnc`/`daid` (DACP-ID) and `ssnc`/`acre` (Active-Remote token), resolves the sender's control endpoint via `avahi-browse` (`iTunes_Ctrl_<DACP-ID>`, `_dacp._tcp`, IPv4), and sends |net clicks| opposite `volumeup`/`volumedown` commands. Hardware-learned: `setproperty?dmcp.device-volume=<dB>` returns HTTP 200 but iOS ignores it, and **iOS only applies volume commands while playing** — the undo is therefore deferred until the `prsm`/`pbeg` after tuning (held while paused, discarded on a new session). A suppression window prevents the undo's own `pvol` echoes from being counted as new tuning clicks.
